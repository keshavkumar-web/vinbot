#!/usr/bin/env bash
# =============================================================================
#  UHBVN Assistant — Production deployment: 2026-07-05 release
# =============================================================================
#  Applies ALL of today's (2026-07-05) updates:
#    A. Transformer "failure rate" now reliably returns the DAMAGE RATE
#       (deterministic damaged_transformers route; "failure rate"->"damage
#       rate"; raw-input guard) instead of an intermittent "could not find".
#    B. A single-term DEFINITION question ("what is PT?", "what is ABCXYZ?")
#       is no longer hijacked by a stale numeric turn into a "name the circle"
#       clarify: known abbr -> glossary, unknown -> clean not-found.
#    (Cumulative 2026-07-02 items below still apply.)
...
#
#  Usage (on the server):
#     scp uhbvn_deploy_2026-07-05.tar.gz ecadmin@<SERVER>:/tmp/      # from dev box first
#     bash deploy_2026-07-05.sh [/path/to/uhbvn_deploy_2026-07-05.tar.gz]
# =============================================================================
set -euo pipefail
...
TARBALL="${1:-/tmp/uhbvn_deploy_2026-07-05.tar.gz}"
STAMP="$(date +%F_%H%M%S)"

echo "==> UHBVN 2026-07-05 deploy"
...
echo ""
echo "==> DONE.  Expected gate: eval_tables 27/27, eval_rag 7/7, eval_followup 30/30, validate_e2e ALL GREEN (0% FP)."
